...
<head>
    ...
    <title>Postify - User Profile</title>
    <!-- Tailwind CSS CDN -->
...
<body class="bg-gray-100 flex flex-col items-center min-h-screen">
        ...
        <div class="container mx-auto flex justify-between items-center">
            ...
            <div class="flex items-center space-x-4">
                <c:if test="${currentUser != null}">
                    <span class="text-gray-700">Welcome, <strong>${currentUser.username}</strong>!</span>
    ...
    <main class="container mx-auto p-4 max-w-4xl w-full">
        ...
        <div class="profile-card p-6 mb-6 text-center">
            <div class="w-24 h-24 bg-gray-300 rounded-full mx-auto flex items-center justify-center text-gray-600 font-bold text-5xl mb-4">
                <i class="fas fa-user"></i>
            </div>
            <h1 class="text-3xl font-bold text-gray-800 mb-2">${profileUser.username}</h1>
            <p class="text-gray-600 mb-4">Joined: <fmt:formatDate value="${profileUser.createdAt}" pattern="MMM dd,yyyy" /></p>
            <c:if test="${profileUser.email != null && !profileUser.email.isEmpty()}">
                <p class="text-gray-500"><i class="fas fa-envelope mr-2"></i>${profileUser.email}</p>
            </c:if>

            <c:if test="${currentUser != null && currentUser.userId == profileUser.userId}">
                <div class="mt-6">
                    <a href="${pageContext.request.contextPath}/editProfile?userId=${currentUser.userId}" class="bg-blue-500 hover:bg-blue-600 text-white font-semibold py-2 px-4 rounded-lg shadow-sm transition ease-in-out duration-200 action-button">
        ...
        <h2 class="text-2xl font-bold text-gray-800 mb-4">Posts by ${profileUser.username}</h2>
        ...
        <div class="space-y-6">
            <c:choose>
                            ...
                            <c:if test="${post.imageUrl != null && !post.imageUrl.isEmpty()}">
                                <div class="mb-4 rounded-lg overflow-hidden">
                                    <img src="${post.imageUrl}" alt="Post Image" class="w-full h-auto object-cover max-h-96"
                                         onerror="this.onerror=null;this.src='https://placehold.co/600x400/CCCCCC/000000?text=Image+Unavailable';">
                                </div>
                            </c:if>
                                            ...
                                            <c:forEach var="comment" items="${post.comments}">
                                                <div class="bg-gray-50 p-3 rounded-lg">
                                                    <div class="flex items-center mb-1">
                                                        <strong class="text-sm text-gray-800 mr-2">${comment.username}</strong>
                ...
                <c:otherwise>
                    <p class="text-center text-gray-500 text-lg py-10">This user hasn't made any posts yet.</p>
                </c:otherwise>
            </c:choose>
        </div>
    </main>

    <!-- Comment Modal (reused from index.jsp) -->
    ...
    <!-- Share Modal (reused from index.jsp) -->
    ...
    <script>
        // Modal functions (reused from index.jsp)
